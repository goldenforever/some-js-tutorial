...
<script>
    ...
    window.maxTask = -1;
    window.tasks = [{"task":"<strong>some-help:</strong> Let's start with making a small article for The Paw. \n\n<span style=\"color: #999\">Note: You can resize the window below by <b>dragging the bottom-right corner</b> down. You can also change the width of the textbox by using the <b>slider above the editor</b>.</span>\n\n<u>First, we'll need a simple header and paragraph.</u>\nYou can create headers with 1 to 6 '#' characters, a space and then the appropriate text.\n\nTry making a header for the article that reads: \n<em>'Paw Treats Coders Like Dogs'</em>.","regex":"#{1,6}\\s+[pP]aw [tT]reats [cC]oders [Ll]ike [Dd]ogs","replace":""},{"task":"<strong>some-help:</strong>\n<u>Right, now let's put your name on it.</u>\n\nMake another smaller header that says: \n<em>'By [your name here].'</em>.\n\nRemember, you can make a smaller header by adding more '#'s before the text.\n\n(Don't forget to include the '.')","regex":"#{2,6}\\s+[bB]y [A-Za-z-' ]*?\\.\\s*?(\\n|$)","replace":""},{"task":"<strong>some-help:</strong>\n<u>Let's make the article title bold.</u>\n\nYou can make something bold by enclosing it within '*' characters. \n<em>'*This is bold text*'</em>.","regex":"\\*\\s*[pP]aw [tT]reats [cC]oders [Ll]ike [Dd]ogs\\s*\\*","replace":""},{"task":"<strong>some-help:</strong>\n<u>Nice one. Now, let's make your name appear in italics.</u>\n\nYou can make something italic by enclosing it between '//'s. \n<em>'//This is italic text//'</em>.","regex":"\\/\\/\\s*([bB]y )?[A-Za-z-' ]*?\\.?\\s*\\/\\/","replace":""},{"task":"<strong>some-help:</strong>\n<u>Now, let's actually write the article.</u>\n\nI've started it for you. As you can see, a blank line between two bodies of text will separate them into paragraphs.\n\nBut it seems that the address doesn't display on separate lines, as it thinks it's all part of one paragraph.\n\n<u>Separate the lines of the 'Dogentry College' address by turning\nthem into separate paragraphs.</u>","regex":"The Paw.*?\\n+.*?\\n+.*?Dogentry Student Union.*?\\n+.*?\\n+.*?Kennelworth.*?\\n+.*?\\n+.*?D[O0]8","replace":"# *Paw Treats Coders Like Dogs*\n\n### By //~[Replace Me]~//.\n\nThe Paw, as you well know, is a newspaper at\nthe prestigious university *Dogentry College*.\n\nUnfortunately the student newspaper is run by\na scary lady who _destroys all_ in her path,\n//especially coders//, as she forces them to\nwork non-stop, not to mention eat dogfood\n(which is surprisingly nutritious).\n\nPlease leave them complaints:\n\n*By post:*  \n\nThe Paw\nDogentry Student Union\nKennelworth\nDO8 8ED\n\n*By telephone:* 07700 WOOFED\n\n*By email:* thepaw@dogentry.ac.uk"},{"task":"<strong>some-help:</strong>\n\nBut this looks a bit weird now. Each line is far from the next.\n\nFortunately there's a nice way around this, called line-breaks.\n\nYou can make a line fold down below by adding two spaces at the end of the line:\n<em><span style=\"background-color:#666\">This is above,  \nthis is below.</span></em>\n\n<u>Change the address to now use line-breaks <strong>instead</strong> of paragraphs.\nMake sure to remove the lines you added in the last exercise.</u>","regex":"The Paw  +\\nDogentry Student Union  +\\nKennelworth  +\\n","replace":""},{"task":"<strong>some-help:</strong>\n<u>Underlines next.</u>\n\nUnderlines are achieved by putting underscores (_) around the text.\n\n<em>'_This is underlined_'</em>.\n\nUnderlines in some-js are more than meet the eye. For instance, <em>'js'</em> in <em>'some_js_tutorial'</em> will not become underlined; and rightly so, it's my username!\n\n<span style=\"color: #999\">i.e. It will only match when spaces/punctuation are on<span style=\"background-color:#666\"> </span>_both outsides_<span style=\"background-color:#666\">.</span></span>\n\n<u>Show off your underlining skills by underlining the first line of the address, 'The Paw'</u>","regex":"_The Paw_  +\\nDogentry","replace":""},{"task":"<strong>some-help:</strong>\n\nThe Paw really does treat coders bad, so how about a way to express our anger?\n\n<strong>Enter emojis!</strong> You can add an emoji with the `{emoji()emoji-code}`\n\n<u>Add an emoji of a newspaper within the article.</u>\n\nThe emoji code for a newspaper is, appropriately, newspaper.","regex":"\\{((emoji)|(emoticon))\\(\\)newspaper\\}","replace":""},{"task":"<strong>some-help:</strong> Nice work! That was your first <strong>general web object</strong>.\n\nA general web object looks like this: <strong>{objectname(setting1,setting2,<span style=\"color:#666\">...</span>)value1,value2,<span style=\"color:#666\">...</span>}</strong>\n\nDifferent objects have different numbers of different settings. Every object has different settings. Emojis don't have settings.\n\nEmoji objects do take as many <em>values</em> as you like, in this case emojicodes, and therefore emojis themselves.\n\n<u>Add <strong>dog</strong> and <strong>rage</strong> face emojis to the same {emoji()...}.\nSeparate values with commas, but <strong>no spaces</strong>.</u>","regex":"\\{((emoji)|(emoticon))\\(\\)(newspaper,)?((dog,rage)|(rage,dog))\\}","replace":""},{"task":"<strong>some-help:</strong> The reason it's called the general web object is because its... well, general!\n\nThere are loads of different ones. Some are just the same as ones you already know, like:\n- {bold()<strong>Bold text</strong>}\n- {italic()<em>Italic text</em>}\n- {underline()<u>Underlined text</u>}\n\n<u>Try the </u>{color(green)<span style=\"color:green\">Green text</span>}<u> and the </u>{font(Arial)<span style=\"font-family:Arial;\">Ooh Arial</span>}","regex":"((\\{color\\(.*?[^\\\\]\\).*?\\}(.|\\n)*\\{font\\(.*?[^\\\\]\\).*?\\})|(\\{font\\(.*?[^\\\\]\\).*?\\}(.|\\n)*\\{color\\(.*?[^\\\\]\\).*?\\}))","replace":""},{"task":"<strong>some-help:</strong> Now we've finished the article, let's finish off the website and then we're done! <i class=\"__emoji__\"><img class=\"emojione\" alt=\"😄\" src=\"//cdn.jsdelivr.net/emojione/assets/png/1f604.png?v=2.1.3\"></i>\n\nHeaders are more important than they seem in <strong>some-js</strong>. They define the structure of a page. A header is considered to 'contain' anything following it that is not a larger (or equally sized) header.\n\nTo get a better idea of the structure, I've added a {contents()} object to the webpage at the top.\n\n<u>Have a look at the contents and see where it lines up with the updated article.\nWhen you're done, remove the contents object from the code.</u>","regex":"^\\s*# The Paw\\s+#","replace":":D"}];

    window.lastSuccess = "";

    function check() {
        if (window.currentTask > -1 && !hidden && cm.getValue().match(new RegExp(window.tasks[window.currentTask].regex), 'gi')) {
    ...
    function nextOne() {
        ...
        if (window.maxTask < window.currentTask-1) {
            ...
            if (window.tasks[window.currentTask].replace) {
                ...
                var name = cm.getValue().match(/(\/\/)?[B]y(\/\/)? (\/\/)?[A-Za-z-' ]*(?=[\/\.])/);
                if (name == null) name = ['     Joe Bloggs'];
                if (window.currentTask == 4) {
                    rep = rep.replace("~[Replace Me]~", name[0].substring(5).trim());
                } else if (window.currentTask == 10) {
